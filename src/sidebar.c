...
void draw_color_palette(SDL_Renderer *renderer, PaintContext *context, Config *config) {
    ...
    for (int i = 0; i < config->palette_count; i++) {
        ...
        SDL_SetRenderDrawColor(renderer, 
            config->palette_colors[i].r, 
            config->palette_colors[i].g, 
            config->palette_colors[i].b, 
            config->palette_colors[i].a);
        ...
        if (context->current_tool.type != TOOL_ERASER &&
            context->current_tool.color.r == config->palette_colors[i].r &&
            context->current_tool.color.g == config->palette_colors[i].g &&
            context->current_tool.color.b == config->palette_colors[i].b) {
            SDL_SetRenderDrawColor(renderer, 255, 255, 0, 255);
...
void handle_color_palette_click(PaintContext *context, Config *config, int mouse_x, int mouse_y) {
    ...
    for (int i = 0; i < config->palette_count; i++) {
        ...
        if (mouse_x >= x && mouse_x <= x + COLOR_PALETTE_SIZE &&
            ...
            if (context->current_tool.type != TOOL_ERASER) {
                context->current_tool.color = config->palette_colors[i];
            }
            return;
        }
    }
